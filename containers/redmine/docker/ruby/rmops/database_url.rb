...
class RMOps::DatabaseURL
  ...
  DBSpec = Struct.new(:type, :uri_user, :user, :pass, :host, :port, :name, :params, :ssl, :single, :env)
  ...
  def initialize(url, **opts)
    ...
    @db.single = opts.fetch(:single, RMOps::Consts::DATABASE_SINGLE)
    @db.env = {}
    if @db.single && @db.host =~ /\.database\.azure\.com$/
